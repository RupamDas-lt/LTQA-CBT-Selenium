...
Feature: Automation of sonoma machine with different browsers.
  ...
  Scenario Outline: User is able to run local test session and run session with tunnel for sonoma browser to test console,command log and video with network false
    ...
    Examples:
      | capabilities                                                                                                                                                                |
      | browserName=chrome,platform=sonoma,version=.*,resolution=.*,timezone=.*,visual=true,network=false,network.http2=false,tunnel=true,console=true                              |
      | browserName=edge,platform=sonoma,version=.*,resolution=.*,timezone=.*,visual=true,network=false,network.http2=false,tunnel=true,selenium_version=.*,seCdp=true,console=true |
  ...
  @sonoma_regression_8 @extension_verification
  Scenario Outline: User is able to run test session with Extension on Sonoma
    Given Setup user details
