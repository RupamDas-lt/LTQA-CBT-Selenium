...
Feature: Automation of ubuntu-20 machine with different browsers.
  ...
  Scenario Outline: User is able to run local test session and run session with tunnel for ubuntu-20 browser to test console,command log and video with network false
    ...
    Examples:
      | capabilities                                                                                                                                   |
      | browserName=chrome,platform=ubuntu,version=.*,resolution=.*,timezone=.*,visual=true,network=false,network.http2=false,tunnel=true,console=true |

    @london_smoke_new
    Examples:
      | capabilities                                                                                                                                                                |
      | browserName=edge,platform=ubuntu,version=.*,resolution=.*,timezone=.*,visual=true,network=false,network.http2=false,tunnel=true,selenium_version=.*,seCdp=true,console=true |
  ...
  @ubuntu_regression_8 @extension_verification
  Scenario Outline: User is able to run test session with Extension on Ubuntu
    Given Setup user details
